...
volatile unsigned int Tpp = 255; // Начальная длительность задержки сигнала в микросекундах (один полупериод)
volatile byte Tb = 9; // Дополнительная задержка сигнала на начало байта в микросекундах
unsigned int Kvkt = 54; // Коэффициент скорости ROM Вектора
unsigned int Tbyte = 264; // Значение для расчёта задержки перед каждым байтом
unsigned int TppSTD = 365;  //376; // Стандартная длительность задержки сигнала для файлов BAS/MON/ASM...
...
const byte M_setup_in1 = 12;
const byte M_setup_in2 = 22;
...
void loop() {
  ...
  switch (MLevel) // действия в соответствии с текущим пунктом меню
  {
    ...
    case M_play_in: // зашли в меню вопроизведения
      switch (button)
      {
        ...
        case BT_left: // в корень или выход
          ...
          else {        // возврат в предыдущую директорию
            cdUp();
          }
          break;
        ...
        case BT_right: // вход в директорию, или запуск файла на воспроизведение
          if (isDir) { //Если это директория, то переход в неё
            ...
            if (Num < 15) {
              if (pathIndex[Num] != dirIndex) {
                pathIndex[Num] = dirIndex;
                for (byte i = Num + 1; i <= 15; i++) {
                  pathIndex[i] = 0;
                }
              }
              Num++;
            }
            dirIndex = pathIndex[Num];
            NextFile();
            PrevFile();
          }
    ...
    case M_record_in: // зашли в меню записи
      ...
      break;
    case M_setup_in1: // зашли в меню настроек
      lcd.setCursor(0, 1);
      lcd.print(F("R/W Const.: "));
      switch (button)
      {
        case BT_up:   // вверх
          if (Kvkt < 80) {
            Kvkt++; // увеличиваем скорость
            Tpp = (Kvkt * 52 - 250) / 10;
          }
          break;
        case BT_down: // вниз
          if (Kvkt > 32) {
            Kvkt--; // уменьшаем скорость
            Tpp = (Kvkt * 52 - 250) / 10;
          }
          break;
        case BT_right:
      ...
      clrstr(lcd.print(Kvkt, HEX)); // пришем текущее значение скорости с очисткой строки до конца
      break;
    case M_setup_in2: // зашли в меню настроек
      lcd.setCursor(0, 1);
      lcd.print(F("Byte delay: "));
      switch (button)
      {
        case BT_up:   // вверх
          if (Tbyte < 280) {
            Tbyte++; // увеличиваем Tbyte
          }
          break;
        case BT_down: // вниз
          if (Tbyte > 200) {
            Tbyte--; // уменьшаем Tbyte
          }
          break;
        case BT_left: // влево
          MLevel = M_setup_in1;
      ...
      clrstr(lcd.print(Tbyte)); // пришем текущее значение Tbyte
      break;
  }
}
...
{
  if (Tpp <= Tbyte) {   // для полупериода меньше или равном Tbyte
    Tb = Tbyte - Tpp;
    if (Tb > (Tpp / 2 )) { // Чтобы Tb не был больше 1/2 полупериода
      Tb = Tpp / 2;
    }
  }
  else {
    Tb = 0;           // для полупериода больше Tbyte
  }
}
...
// BLs -- стартовый блок
{
  ...
  if (dataFile.open(sfileName, O_READ)) { // открываем файл. Открылся?
    ...
    lcd.setCursor(13, 0);              // выводим на экран кол-во блоков
    lcd.print('/');
    if (BLe < 16) lcd.print('0');
    lcd.print(BLe, HEX);
    ...
    for (BLt = BLe; BLt >= 1; BLt--) { // Вывод блоков данных в цикле
      ...
      lcd.setCursor(10, 0);              // выводим на экран кол-во оставшихся блоков
      if (BLt < 100) {
        lcd.print(' ');
        if (BLt < 10) lcd.print(' ');
      }
