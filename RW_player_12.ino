...
volatile byte Tb = 9; //16;  // Дополнительная задержка сигнала на начало байта в микросекундах
...
void loop() {
  ...
  switch (MLevel) // действия в соответствии с текущим пунктом меню
  {
    ...
    case M_setup_in: // зашли в меню настроек
      ...
      lcd.setCursor(12, 1);
      lcd.print(Kvkt, HEX); // пришем текущее значение скорости
      break;
  }
}
...
void CalcTb()  // Вычисление значения задержки на начало байта Tb
{
  if (Tpp <= 176) {     // для полупериода меньше или равном 176
    ...
    if (Tpp <= 264) {   // для полупериода от 177 до 261
      Tb = 264 - Tpp;
    }
    else {
      Tb = 0;           // для полупериода больше 264
    }
  }
}
